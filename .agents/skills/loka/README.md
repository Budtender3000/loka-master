...
- **Safe Mutation and Rollback:** Executes mutations through a dedicated worker (`loka-writer`) and transactional engine (`scripts/loka.py mint`) with realpath containment within `./.agents/loka-brain/`, executing mechanical audits and catalog indexing, with automated rollbacks on any failure.
...
- **Automated Mechanical Auditing:** Verifies frontmatter schemas, domain classification, path leakage, credential patterns, heading structures, and wikilink integrity via `scripts/loka.py audit`.
...
| `scripts/loka.py` | Unified CLI entry point (`format`, `index`, `audit`, `mint`, `promote`). | `scripts/loka.py` |
